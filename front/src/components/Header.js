...
function Header() {
    ...
    if (!isAuthenticated) {
      return (
        <div className="hidden md:flex items-center gap-4">
          <svg
            className={`h-6 w-6 text-black transition-all duration-500 ${isScrolled ? "invert" : ""}`}
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            viewBox="0 0 24 24"
  ...
  return (
    ...
    >
      {/* Logo */}
      <a
        href="https://prebuiltui.com shrink-0"
        className="flex items-center w-50 h-50 gap-2"
      >
        <img
          className="h-32 object-contain filter brightness-0"
          src="/logo.png"
          alt="Logo"
        />
      </a>
      ...
      <div className="hidden md:flex items-center gap-4 lg:gap-8">
        {navLinks.map((link, i) => (
          <a
            key={i}
            href={link.path}
            className={`group flex flex-col gap-0.5 ${isScrolled ? "text-gray-700" : "text-gray-700"}`}
          >
            {link.name}
            <div
              className={`${isScrolled ? "bg-gray-700" : "text-gray-700"} h-0.5 w-0 group-hover:w-full transition-all duration-300`}
            />
          </a>
        ))}
      </div>
      ...
      >
        ...
        <button
          className={`  ml-0 md:ml-4  ${isScrolled ? "text-white bg-[#F86D72]" : "bg-[#F86D72] text-white"}`}
        >
          Войти
        </button>
      </div>
