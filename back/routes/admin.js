...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      await newBook.save();
      res.status(201).json({ message: "Товар создан", book: newBook });
    } catch (error) {
...
router.get("/getBooks", cookieAuth, async (req, res) => {
  try {
    if (req.user.role !== "admin") {
      return res.status(403).json({ message: "Доступ запрщен. Вы не админ." });
    }
...
router.put("/updateBook/:id", async (req, res) => {
  try {
    ...
    res.json({ message: "Товар обновлен", book });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
